...
export const apiAddItems   = (names)      => api(`/api/items?chat_id=${CHAT_ID}&action=add`,     'POST', { names, added_by: ME });
export const apiTakeItem   = (id)         => api(`/api/items?chat_id=${CHAT_ID}&action=take`,    'POST', { id, user: ME });
export const apiReleaseItem= (id)         => api(`/api/items?chat_id=${CHAT_ID}&action=release`, 'POST', { id });
export const apiBuyItem    = (id)         => api(`/api/items?chat_id=${CHAT_ID}&action=buy`,     'POST', { id, user: ME });
export const apiDeleteItem = (id)         => api(`/api/items?chat_id=${CHAT_ID}&action=delete`,  'POST', { id });
